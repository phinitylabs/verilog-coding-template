...
- Actions controlled by flags:
  ...
  --json/-j: Generate tasks.py for run_eval.py
...
from hud_controller.prompts import spec_to_statement
...
TASK_TEMPLATE_ID = "solve_problem"


def generate_tasks_py(specs: list[ProcessedSpec], output_path: str = "tasks.py") -> None:
    """Generate a portable tasks.py rows file for run_eval.py."""
    lines = [
        '"""Auto-generated by imagectl3 -j. Do not edit."""',
        "",
    ...
    for spec in specs:
        ...
        lines.append(f'        id="{TASK_TEMPLATE_ID}",')
        lines.append(f'        slug="{spec.id}",')
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    logger.info("Generated %s with %d task(s)", output_path, len(specs))
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        "--json",
        action="store_true",
        help="Generate tasks.py rows file for run_eval.py",
    )
    parser.add_argument(
